...
impl Tool for BrowserTool {
    ...
    fn description(&self) -> &str {
        concat!(
            ...
            "   - 'text' → headings (h1-h6) for page structure and context\n",
            "   - 'buttons' → action buttons (add to cart, buy, submit, confirm)\n",
            "   - 'inputs' → text fields, search boxes, dropdowns\n",
            "   - 'links' → navigation links with [section] context from nearest heading\n",
            "   - 'other' → alerts, status messages, misc interactive elements\n",
            "   Note: [disabled] elements cannot be clicked. [checked] means already selected.\n",
