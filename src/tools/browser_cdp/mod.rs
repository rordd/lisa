...
/// Wait for page content to fully render using MutationObserver.
/// Watches for DOM changes and waits until no mutations occur for `idleTime` ms.
/// Falls back to timeout after `maxWait` ms.
/// This handles SPA sites where content loads via API calls after initial page load.
const DOM_STABILIZE_JS: &str = r#"
    new Promise((resolve) => {
        ...
        // Initial timer: page may already be stable
        timer = setTimeout(() => {
            observer.disconnect();
            clearTimeout(timeoutTimer);
            resolve('stable');
            ...
            BrowserAction::Find {
                ...
                let find_js = match by.as_str() {
                    ...
                    "testid" | "test_id" => format!(
                        "(function() {{ var val = {}; var el = document.querySelector('[data-testid=\"' + val + '\"]') || document.querySelector('[data-test-id=\"' + val + '\"]'); if (el) {{ el.setAttribute('data-cdp-found', '1'); return true; }} return false; }})()",
                        safe_value
                    ),
