...
| | **OpenClaw** | **ZeroClaw (upstream)** | **Lisa (우리)** |
|---|---|---|---|
| 언어 | Node.js | Rust | Rust |
| 브라우저 엔진 | **Playwright** (내장) | agent-browser CLI (Node.js) | **chromiumoxide** (CDP 직접) |
| 프로토콜 | Playwright API → 내부적으로 CDP | Playwright CLI 래핑 | **CDP WebSocket 직접** |
| 의존성 | Playwright + Chromium 번들 | Node.js + agent-browser npm | chromiumoxide crate만 |
| 바이너리 추가 | ~200MB (Chromium) | ~100MB (Node.js+npm) | **0** (시스템 Chrome/WAM 사용) |
| 백엔드 수 | 1 (Playwright) | 4 (agent_browser, rust_native, computer_use, auto) | 5 (+cdp_direct) |
...
|---|---|---|---|
| 데이터 소스 | **접근성 트리 (AX Tree)** | DOM 직접 순회 JS | DOM 직접 순회 JS |
| API | `page.accessibility.snapshot()` / `_snapshotForAI()` | `snapshot_script()` JS evaluate | `snapshot_script()` JS evaluate |
| 필터링 | 17개 INTERACTIVE_ROLES 셋 | 6개 태그 + `el.onclick` | 6개 태그 + `el.onclick` |
| 포맷 | 들여쓰기 트리 + `[ref=e1]` | 카테고리 그룹핑 (buttons/inputs/links/other) | 카테고리 그룹핑 |
| compact 모드 | ✅ (구조적 노드 제거) | ✅ (텍스트 없는 비인터랙티브 제거) | ✅ |

**OpenClaw이 정확한 이유:** Playwright가 브라우저의 접근성 엔진을 직접 쓴다. OS 스크린리더가 쓰는 것과 같은 데이터. 브라우저가 이미 계산한 role을 가져오니까, 개발자가 `<div onClick>` 써도 브라우저가 role을 추론해서 잡힌다.

**ZeroClaw/Lisa(현재)의 한계:** DOM을 직접 걸어다니면서 태그 이름으로 판단. `addEventListener`로 등록된 이벤트, React/Vue synthetic event(root에 위임)는 못 잡는다.

## 1.4 인터랙티브 감지 비교

"클릭 가능한 요소"를 찾는 정확도. 이게 스냅샷 품질을 결정한다.

| | **OpenClaw** | **ZeroClaw** | **Lisa (현재)** | **Lisa (계획)** |
|---|---|---|---|---|
| 1차 | AX Tree role 기반 | 태그/속성 체크 | 태그/속성 체크 | **AX Tree** (CDP API) |
| 2차 | — | compact 모드 (텍스트 유무) | compact 모드 | **DOM 휴리스틱** |
| 3차 | — | — | — | **L3 Vision 폴백** |
| React `<div onClick>` | ✅ | ❌ | ❌ | ✅ (AX Tree) |
| 접근성 미대응 `<div class="btn">` | ❌ | ❌ | ❌ | ✅ (cursor:pointer) |
| 커버리지 | ~90% | ~70% | ~70% | **~95%** |

**OpenClaw도 못 잡는 것:** `<div class="btn" style="cursor:pointer">` 같이 role도 aria도 없는 요소. Lisa의 3단계 전략이 이걸 커버하는 유일한 방법.
